...
@section('content')
    ...
    <main class="flex-1 overflow-y-auto bg-gradient-to-br from-gray-50 to-gray-100 p-3 sm:p-4 lg:p-6">
        ...
        <div class="mb-6 bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-lg">
            ...
            <p class="text-sm text-yellow-700 mt-1">Le lot <strong>{{ $attribution->lot->numero ?? 'N/A' }}</strong> sera réattribué avec une nouvelle proforma. L'ancienne attribution devient historique.</p>
        </div>

        <form action="{{ route('attributions.reattribuer', $attribution->id_attribution) }}" method="POST" id="reattributionForm">
            @csrf
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="px-6 py-4 bg-gradient-to-r from-red-50 to-white border-b">
                            <h2 class="text-lg font-bold text-gray-800"><i class="fas fa-exclamation-circle text-red-500 mr-2"></i>Motif de réattribution</h2>
                        </div>
                        <div class="p-6">
                            <textarea name="motif_reattribution" rows="3" required minlength="10" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-red-400" placeholder="Pourquoi réattribuez-vous ce lot ? (minimum 10 caractères)">{{ old('motif_reattribution') }}</textarea>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="px-6 py-4 bg-gradient-to-r from-green-50 to-white border-b">
                            <h2 class="text-lg font-bold text-gray-800"><i class="fas fa-user-plus text-green-500 mr-2"></i>Nouveau prestataire</h2>
                        </div>
                        <div class="p-6 space-y-5">
                            <div>
                                {{-- {{ dd($prestataires) }} --}}
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Sélectionner le prestataire <span class="text-red-500">*</span></label>
                                <select name="prestataire_id" id="prestataire_id" required class="tom-select-prestataire w-full">
                                    ...
                                    @foreach($prestataires as $p)
                                        <option value="{{ $p->id_prestataire }}"
                                            ...
                                            @if($p->ville_prestataire) - {{ $p->ville_prestataire }}@endif
                                            @if($p->id_prestataire == $attribution->prestataire_id) (Ancien prestataire)@endif
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div id="prestataire_info_card" class="hidden bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 p-5">
                                <div class="flex items-start justify-between mb-4">
                                    <h3 class="font-semibold text-green-800"><i class="fas fa-building mr-2"></i>Informations du prestataire</h3>
                                    <span id="badge_ancien" class="hidden px-3 py-1 bg-orange-100 text-orange-700 text-xs font-medium rounded-full">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="px-6 py-4 bg-gradient-to-r from-purple-50 to-white border-b">
                            <h2 class="text-lg font-bold text-gray-800"><i class="fas fa-file-invoice text-purple-500 mr-2"></i>Nouvelle Proforma</h2>
                            <p class="text-sm text-gray-500 mt-1">Une nouvelle proforma sera créée pour cette réattribution</p>
                        </div>
                        <div class="p-6">
                            <div class="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl border border-purple-200 overflow-hidden">
                                <div class="px-5 py-4 bg-purple-100/50 border-b border-purple-200">
                                    <h3 class="font-semibold text-purple-800"><i class="fas fa-file-medical mr-2"></i>Informations de la proforma</h3>
                                    <p class="text-xs text-purple-600 mt-1">Champs * obligatoires</p>
                                </div>
                                <div class="p-5 space-y-5">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div>
                                            <label class="block text-sm font-semibold text-gray-700 mb-2">Numéro proforma</label>
                                            ...
                                            <p class="text-xs text-gray-500 mt-1"><i class="fas fa-magic mr-1"></i>Format auto: PROF-{{ date('Y') }}-0001</p>
                                        </div>
                                        <div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        ...
                                        <div>
                                            <label class="block text-sm font-semibold text-gray-700 mb-2">Montant hors taxe HT (FCFA) <span class="text-red-500">*</span></label>
                                            <input type="number" name="new_montant_retenu" id="new_montant_retenu"
                                                value="{{ old('new_montant_retenu', 0) }}" min="0" required
                                                class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-purple-400">
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div class="bg-white/60 p-4 rounded-lg border border-purple-100">
                                            ...
                                            <div>
                                                ...
                                                <div class="relative">
                                                    <input type="number" id="new_tva_pourcentage" value="{{ old('new_taux_tva', 18) }}"
                                        ...
                                        <div class="bg-white/60 p-4 rounded-lg border border-purple-100">
                                            ...
                                            <div>
                                                ...
                                                <div class="relative">
                                                    <input type="number" id="new_remise_pourcentage" value="{{ old('new_remise_pourcentage', 0) }}"
                                    ...
                                    <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl p-5 text-white">
                                        <h4 class="font-semibold mb-4"><i class="fas fa-calculator mr-2"></i>Récapitulatif Proforma</h4>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-5">
                            <div>
                                ...
                                <input type="date" name="date_attribution" id="date_attribution" value="{{ old('date_attribution', date('Y-m-d')) }}" max="{{ date('Y-m-d') }}" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-400">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Date début <span class="text-red-500">*</span></label>
                                <input type="date" name="date_debut_prevue" id="date_debut_prevue" value="{{ old('date_debut_prevue', date('Y-m-d')) }}" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-400">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Date fin <span class="text-red-500">*</span></label>
                                <input type="date" name="date_fin_prevue" id="date_fin_prevue" value="{{ old('date_fin_prevue') }}" required class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-400">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-white border-b">
                            <h2 class="text-lg font-bold text-gray-800"><i class="fas fa-comment-alt text-gray-500 mr-2"></i>Observations</h2>
                        </div>
                        <div class="p-6">
                            <textarea name="observations" rows="3" class="w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-gray-400" placeholder="Notes complémentaires...">{{ old('observations') }}</textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="px-6 py-4 bg-gradient-to-r from-indigo-50 to-white border-b">
                            <h2 class="text-lg font-bold text-gray-800"><i class="fas fa-box text-indigo-500 mr-2"></i>Lot concerné</h2>
                        </div>
                        <div class="p-6 space-y-3">
                            <div>
                                <span class="text-sm text-gray-600">Numéro:</span>
                                <span class="inline-flex items-center px-3 py-1 rounded-lg text-sm font-bold bg-indigo-100 text-indigo-700 ml-2">{{ $attribution->lot->numero ?? 'N/A' }}</span>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p class="text-gray-700 text-sm mt-1">{{ $attribution->lot->appelOffre->numero_appel_offre }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="px-6 py-4 bg-gradient-to-r from-gray-100 to-white border-b">
                            <h2 class="text-lg font-bold text-gray-800"><i class="fas fa-history text-gray-500 mr-2"></i>Ancienne attribution</h2>
                        </div>
                        <div class="p-6 space-y-3 text-sm">
                            ...
                            <div class="flex justify-between items-start">
                                ...
                                <span class="font-medium text-right max-w-[150px]">{{ $attribution->prestataire->raison_sociale_prestataire ?? 'N/A' }}</span>
                            </div>
                            @if($attribution->proforma)
                            <div class="flex justify-between"><span class="text-gray-600">Proforma:</span><span class="font-medium">{{ $attribution->proforma->numero_proforma }}</span></div>
                            <div class="flex justify-between"><span class="text-gray-600">Montant TTC:</span><span class="font-medium">{{ number_format($attribution->proforma->montant_ttc ?? 0, 0, ',', ' ') }} FCFA</span></div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        <div class="px-6 py-4 bg-gradient-to-r from-green-50 to-white border-b">
                            <h2 class="text-lg font-bold text-gray-800"><i class="fas fa-coins text-green-500 mr-2"></i>Montant engagé</h2>
                        </div>
                        <div class="p-6">
                            ...
                            <p class="text-xs text-gray-500 mt-2"><i class="fas fa-info-circle mr-1"></i>Sera auto-rempli avec le Total TTC de la proforma</p>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg p-6 space-y-3">
                        ...
                        <button type="submit" class="w-full px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-medium rounded-lg shadow-md flex items-center justify-center transition-all">
                            <i class="fas fa-redo mr-2"></i>Réattribuer le lot
                        </button>
                        @endcan
                        @can('attributions_lots.view-details')
                        <a href="{{ route('attributions.show', $attribution->id_attribution) }}" class="w-full px-6 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 font-medium rounded-lg flex items-center justify-center transition-all">
    ...
    const prestataireSelect = new TomSelect('#prestataire_id', {
        ...
        render: {
            option: function(data, escape) {
                ...
                if (ancien) {
                    html += ' <span class="text-xs bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full ml-2">Ancien</span>';
                }
                html += '</div>';
                if (ville) html += '<div class="text-xs text-gray-500"><i class="fas fa-map-marker-alt mr-1"></i>' + escape(ville) + '</div>';
                if (telephone) html += '<div class="text-xs text-gray-500"><i class="fas fa-phone mr-1"></i>' + escape(telephone) + '</div>';
                if (email) html += '<div class="text-xs text-gray-500"><i class="fas fa-envelope mr-1"></i>' + escape(email) + '</div>';
                html += '</div>';
                return html;
            },
