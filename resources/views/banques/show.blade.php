        ...
        <div class="px-3 sm:px-4 lg:px-6 py-4">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('banques.index', $prestataire->id_prestataire) }}" class="p-2 hover:bg-gray-100 rounded-lg transition-all duration-200">
                    ...
                    <div>
                        <div class="flex items-center space-x-3 flex-wrap">
                            ...
                            @if($banque->code_banque)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-800">{{ $banque->code_banque }}</span>
                            @endif
                            ...
                            @else
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-800"><i class="fas fa-times-circle mr-1"></i> Inactive</span>
                            @endif
                        </div>
                        <p class="text-gray-600 mt-1"><i class="fas fa-building mr-1"></i>{{ $prestataire->raison_sociale_prestataire }}</p>
                    </div>
                </div>
                <div class="flex items-center space-x-2 flex-wrap">
                    <button onclick="window.location.href='{{ route('banques.edit', ['prestataireId' => $prestataire->id_prestataire, 'banque' => $banque->id_banque]) }}'" class="px-4 py-2.5 bg-white border border-orange-300 text-orange-600 hover:bg-orange-50 rounded-lg transition-all duration-200 flex items-center space-x-2 shadow-sm">
                        <i class="fas fa-edit text-sm"></i>
                        <span class="text-sm font-medium">Modifier</span>
                    </button>
                    <button onclick="toggleStatus({{ $banque->actif_banque ? 'true' : 'false' }})" class="px-4 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 rounded-lg transition-all duration-200 flex items-center space-x-2 shadow-sm">
                        <i class="fas fa-power-off text-sm"></i>
                        <span class="text-sm font-medium">{{ $banque->actif_banque ? 'Désactiver' : 'Activer' }}</span>
                    </button>
                    <button onclick="confirmDelete()" class="px-4 py-2.5 bg-white border border-red-300 text-red-600 hover:bg-red-50 rounded-lg transition-all duration-200 flex items-center space-x-2 shadow-sm">
    ...
    <main class="flex-1 overflow-y-auto bg-gradient-to-br from-gray-50 to-gray-100 p-3 sm:p-4 lg:p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="bg-gradient-to-br from-blue-50 to-white p-5 rounded-xl border border-blue-100">
                                ...
                                @if($banque->code_banque)<p class="text-sm text-gray-500 mt-1">Code: {{ $banque->code_banque }}</p>@endif
                            </div>
                            <div class="bg-gradient-to-br from-green-50 to-white p-5 rounded-xl border border-green-100">
                                ...
                                <p class="text-xl font-bold text-gray-900">{{ $banque->titulaire_compte_banque ?? 'Non renseigné' }}</p>
                            </div>
                            <div class="bg-gradient-to-br from-purple-50 to-white p-5 rounded-xl border border-purple-100">
                                ...
                                <p class="text-xl font-bold text-gray-900 font-mono">{{ $banque->numero_compte_banque ?? 'Non renseigné' }}</p>
                            </div>
                            <div class="bg-gradient-to-br from-yellow-50 to-white p-5 rounded-xl border border-yellow-100">
                                ...
                                <p class="text-xl font-bold text-gray-900 font-mono">{{ $banque->code_guichet_banque ?? 'Non renseigné' }}</p>
                            </div>
                        </div>
                            ...
                            <div class="mt-6 p-6 bg-gradient-to-r from-indigo-500 to-indigo-600 rounded-xl text-white">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-2xl font-bold mt-1 font-mono tracking-wider">{{ $banque->rib_complet }}</p>
                                    </div>
                                    <div class="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center">
                                        <i class="fas fa-id-card text-3xl"></i>
                                    </div>
                                </div>
                                @if($banque->cle_rib_banque)<p class="text-indigo-200 text-sm mt-2">Clé RIB: {{ $banque->cle_rib_banque }}</p>@endif
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            @if($banque->iban_banque)
                                <span class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-bold bg-blue-100 text-blue-700 font-mono">{{ $banque->iban_banque }}</span>
                            @else
                        ...
                        <div>
                            ...
                            @if($banque->swift_bic_banque)
                                <span class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-bold bg-purple-100 text-purple-700 font-mono">{{ $banque->swift_bic_banque }}</span>
                            @else
                                <p class="text-gray-500">Non renseigné</p>
                            @endif
                        </div>
                        <div class="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-gray-200">
                            <div class="text-center">
                                <div class="w-12 h-12 mx-auto rounded-full flex items-center justify-center {{ $stats['rib_complet'] ? 'bg-green-100' : 'bg-gray-100' }}">
                                    <i class="fas {{ $stats['rib_complet'] ? 'fa-check text-green-500' : 'fa-times text-gray-400' }}"></i>
                                </div>
                                <p class="text-sm text-gray-600 mt-2">RIB Complet</p>
                            </div>
                            <div class="text-center">
                                <div class="w-12 h-12 mx-auto rounded-full flex items-center justify-center {{ $stats['has_iban'] ? 'bg-green-100' : 'bg-gray-100' }}">
                                    <i class="fas {{ $stats['has_iban'] ? 'fa-check text-green-500' : 'fa-times text-gray-400' }}"></i>
                                </div>
                                <p class="text-sm text-gray-600 mt-2">IBAN</p>
                            </div>
                            <div class="text-center">
                                <div class="w-12 h-12 mx-auto rounded-full flex items-center justify-center {{ $stats['has_swift'] ? 'bg-green-100' : 'bg-gray-100' }}">
                                    <i class="fas {{ $stats['has_swift'] ? 'fa-check text-green-500' : 'fa-times text-gray-400' }}"></i>
                                </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    <div class="px-6 py-4 bg-gradient-to-r from-green-50 to-white border-b border-gray-200 flex items-center justify-between">
                        <h2 class="text-lg font-bold text-gray-800 flex items-center">
                            <i class="fas fa-money-check-alt text-green-500 mr-2"></i>
                            Paiements associés
                        </h2>
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-green-100 text-green-700">{{ $stats['nombre_paiements'] }} paiement(s)</span>
                    </div>
                    <div class="p-6">
                        @if ($banque->paiements && $banque->paiements->count() > 0)
                                ...
                                @foreach ($banque->paiements as $paiement)
                                    <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div>
                                                <p class="font-medium text-gray-900">{{ number_format($paiement->montant_net_paye_paiement, 0, ',', ' ') }} FCFA</p>
                            ...
                            <div class="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="text-lg font-bold text-green-600">{{ number_format($stats['montant_total_paiements'], 0, ',', ' ') }} FCFA</span>
                                </div>
                            </div>
                        @else
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <a href="{{ route('prestataires.show', $prestataire->id_prestataire) }}" class="mt-4 w-full inline-flex items-center justify-center px-4 py-2 bg-purple-50 text-purple-600 rounded-lg hover:bg-purple-100 transition-colors text-sm font-medium">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        @if ($banque->createur)
                            <div class="flex items-center justify-between py-2 border-b border-gray-100">
                                ...
                                <span class="text-sm font-medium text-gray-900">{{ $banque->createur->nom_complet ?? 'N/A' }}</span>
                            </div>
                        @endif
                        <div class="flex items-center justify-between py-2 border-b border-gray-100">
                        ...
                        @if ($banque->modificateur)
                            <div class="flex items-center justify-between py-2">
                                ...
                                <span class="text-sm font-medium text-gray-900">{{ $banque->modificateur->nom_complet ?? 'N/A' }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                    ...
                    <div class="p-4 space-y-2">
                        <button onclick="duplicate()" class="w-full flex items-center justify-between p-3 text-gray-700 hover:bg-purple-50 rounded-lg transition-colors group">
                            <span class="flex items-center"><i class="fas fa-copy text-purple-500 mr-3"></i>Dupliquer</span>
                            <i class="fas fa-chevron-right text-gray-400 group-hover:text-purple-500"></i>
                        </button>
                        <button onclick="window.print()" class="w-full flex items-center justify-between p-3 text-gray-700 hover:bg-gray-50 rounded-lg transition-colors group">
                            <span class="flex items-center"><i class="fas fa-print text-gray-500 mr-3"></i>Imprimer</span>
                            <i class="fas fa-chevron-right text-gray-400 group-hover:text-gray-600"></i>
                        </button>
                        <button onclick="window.location.href='{{ route('banques.edit', ['prestataireId' => $prestataire->id_prestataire, 'banque' => $banque->id_banque]) }}'" class="w-full flex items-center justify-between p-3 text-gray-700 hover:bg-orange-50 rounded-lg transition-colors group">
                            <span class="flex items-center"><i class="fas fa-edit text-orange-500 mr-3"></i>Modifier</span>
    ...
    <script>
        ...
        window.toggleStatus = function(isActive) {
            ...
            if (confirm(`Voulez-vous vraiment ${action} cette banque ?`)) {
                fetch(`/${prestataireId}/banques/${banqueId}/toggle-statut`, {
                    method: 'PATCH',
        ...
        window.duplicate = function() {
            if (confirm('Voulez-vous dupliquer cette banque ?')) {
                fetch(`/${prestataireId}/banques/${banqueId}/dupliquer`, {
                    method: 'POST',
                    headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json', 'Accept': 'application/json' }
                })
                .then(r => r.json())
                .then(data => { if (data.success) window.location.href = `/${prestataireId}/banques/${data.data.id_banque}/edit`; else alert(data.message || 'Erreur'); })
                .catch(() => alert('Une erreur est survenue'));
            }
        }
