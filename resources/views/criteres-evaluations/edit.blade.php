...
@section('breadcrumb')
    ...
    <a @can('appels_offres.view-details') href="{{ route('appels-offres.show', $lot->appelOffre->id_appel_offre) }}" @endcan
        class="text-white/80 hover:text-white transition-colors">{{ $lot->appelOffre->numero_appel_offre }}</a>
    <i class="fas fa-chevron-right text-white/50 text-xs mx-2"></i>
    ...
    <a @can('lots.view-details') href="{{ route('lots-appels-offres.show', [$critere->lot->appelOffre->id_appel_offre, $critere->lot->id_lot]) }}" @endcan
        class="text-white/80 hover:text-white transition-colors">{{ $critere->lot->numero }}</a>
    <i class="fas fa-chevron-right text-white/50 text-xs mx-2"></i>
    <a @can('criteres_evaluations.read') href="{{ route('criteres-evaluations.index', [$lot->appelOffre->id_appel_offre, $lot->id_lot]) }}" @endcan
        class="text-white/80 hover:text-white transition-colors">Critères</a>
    <i class="fas fa-chevron-right text-white/50 text-xs mx-2"></i>
    <a @can('criteres_evaluations.view-details') href="{{ route('criteres-evaluations.show', [$lot->appelOffre->id_appel_offre, $lot->id_lot, $critere->id_critere_evaluation]) }}" @endcan
        class="text-white/80 hover:text-white transition-colors">{{ $critere->numero_critere_evaluation }}</a>
    <i class="fas fa-chevron-right text-white/50 text-xs mx-2"></i>
...
@section('content')
    ...
    <main class="flex-1 overflow-y-auto bg-gradient-to-br from-gray-50 to-gray-100 p-3 sm:p-4 lg:p-6">
        ...
        @can('criteres_evaluations.update')
            <!-- Formulaire -->
            <form method="POST"
                action="{{ route('criteres-evaluations.update', [$lot->appelOffre->id_appel_offre, $lot->id_lot, $critere->id_critere_evaluation]) }}"
                ...
                <div class="space-y-6">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="flex items-center space-x-4 text-sm text-gray-600">
                                            <span>
                                                ...
                                                Notes des autres critères:
                                                <strong>{{ number_format($totalAutresNotes, 0) }}/100</strong>
                                            </span>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" value="{{ $critere->numero_critere_evaluation }}" disabled
                                    class="w-full px-4 py-2.5 border border-gray-200 bg-gray-50 rounded-lg text-gray-600 cursor-not-allowed">
                            </div>

                            <!-- Ordre d'exécution -->
                            <div>
                                <label for="ordre_execution_critere_evaluation"
                                    class="block text-sm font-semibold text-gray-700 mb-2">
                                    Ordre d'exécution <span class="text-red-500">*</span>
                                </label>
                                <input type="number" id="ordre_execution_critere_evaluation"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" id="libelle_critere_evaluation" name="libelle_critere_evaluation"
                            ...
                            <!-- Description -->
                            <div class="md:col-span-2">
                                <label for="description_critere_evaluation"
                                    class="block text-sm font-semibold text-gray-700 mb-2">
                                    Description
                                </label>
                                <textarea id="description_critere_evaluation" name="description_critere_evaluation" rows="4"
                    ...
                    <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Note de référence -->
                            <div>
                                <label for="note_reference_critere_evaluation"
                                    class="block text-sm font-semibold text-gray-700 mb-2">
                                    Note de référence (sur 100) <span class="text-red-500">*</span>
                                </label>
                                <div class="relative">
                                    <input type="number" id="note_reference_critere_evaluation"
                                        ...
                                        min="0" max="{{ $noteMaximaleAutorisee }}" step="0.01" required
                            ...
                            <div>
                                ...
                                <select id="statut_critere_evaluation" name="statut_critere_evaluation" required
                                    ...
                                    <option value="1"
                                        {{ old('statut_critere_evaluation', $critere->statut_critere_evaluation) == '1' ? 'selected' : '' }}>
                                        Actif</option>
                                    <option value="0"
                                        {{ old('statut_critere_evaluation', $critere->statut_critere_evaluation) == '0' ? 'selected' : '' }}>
                                        Inactif</option>
                                </select>
                        ...
                        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            ...
                            @if ($critere->updated_at && $critere->updated_at != $critere->created_at)
                                <div>
                                    ...
                                    <span
                                        class="font-semibold text-gray-900 ml-2">{{ $critere->updated_at->format('d/m/Y à H:i') }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Modifié par:</span>
                                    <span
                                        class="font-semibold text-gray-900 ml-2">{{ $critere->updater->nom_complet ?? 'N/A' }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
