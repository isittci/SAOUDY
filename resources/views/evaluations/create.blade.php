...
@section('breadcrumb')
    ...
    <a @can('evaluations_attributions.evaluate') href="{{ route('evaluations.pour-attribution', $attribution->id_attribution) }}" @endcan
        class="text-white/80 hover:text-white transition-colors">Attribution</a>
    <i class="fas fa-chevron-right text-white/50 text-xs mx-2"></i>
                        ...
                        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                            ...
                            <div class="p-6">
                                @if ($critereSelectionne)
                                    ...
                                    <input type="hidden" name="critere_id"
                                        value="{{ $critereSelectionne->id_critere_evaluation }}">
                                    <div class="bg-indigo-50 rounded-xl p-4 border border-indigo-200">
                                        <div class="flex items-center justify-between mb-3">
                                            <div class="flex items-center space-x-3">
                                                <span
                                                    class="px-3 py-1 text-sm font-bold bg-indigo-100 text-indigo-700 rounded-lg">
                                                    {{ $critereSelectionne->numero_critere_evaluation }}
                                                </span>
                                                <h4 class="font-semibold text-gray-900">
                                                    {{ $critereSelectionne->libelle_critere_evaluation }}</h4>
                                            </div>
                                        ...
                                        @if ($critereSelectionne->description_critere_evaluation)
                                            <p class="text-sm text-gray-600 mb-3">
                                                {{ $critereSelectionne->description_critere_evaluation }}</p>
                                        @endif
                                        <div class="flex items-center justify-between pt-3 border-t border-indigo-200">
                                            ...
                                            <span
                                                class="text-xl font-bold text-indigo-600">{{ number_format($resteAEvaluer, 2) }}
                                                pts</span>
                                        </div>
                                    </div>
                                @else
                                        ...
                                        @foreach ($criteresAvecReste as $item)
                                            @if ($item['peut_evaluer'])
                                                <label class="block cursor-pointer">
                                                    <input type="radio" name="critere_id"
                                                        value="{{ $item['critere']->id_critere_evaluation }}"
                                                        class="sr-only peer"
                                                        ...
                                                        required>
                                                    <div
                                                        class="p-4 border-2 border-gray-200 rounded-xl transition-all peer-checked:border-indigo-500 peer-checked:bg-indigo-50 hover:border-indigo-300">
                                                        <div class="flex items-center justify-between mb-2">
                                                            <div class="flex items-center space-x-3">
                                                                <span
                                                                    class="px-2 py-1 text-xs font-bold bg-gray-100 text-gray-600 rounded">
                                                                    {{ $item['critere']->numero_critere_evaluation }}
                                                                </span>
                                                                <h4 class="font-semibold text-gray-900">
                                                                    {{ $item['critere']->libelle_critere_evaluation }}</h4>
                                                            </div>
                                                            <span
                                                                class="font-bold text-indigo-600">{{ $item['critere']->note_reference_critere_evaluation }}
                                                                pts</span>
                                                        </div>
                                                        <div class="flex items-center justify-between text-sm">
                                                            <div class="flex items-center space-x-4">
                                                                <span class="text-gray-500">
                                                                    Évalué: <strong
                                                                        class="text-gray-700">{{ number_format($item['total_evalue'], 2) }}
                                                                        pts</strong>
                                                                </span>
                                                                <span class="text-green-600">
                                                                    Reste:
                                                                    <strong>{{ number_format($item['reste_a_evaluer'], 2) }}
                                                                        pts</strong>
                                                                </span>
                                                            </div>
                                                            <div class="w-24 bg-gray-200 rounded-full h-2">
                                                                <div class="h-2 rounded-full bg-indigo-500"
                                                                    style="width: {{ min($item['pourcentage_complete'], 100) }}%">
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            @else
                                                <div class="p-4 border-2 border-gray-200 rounded-xl bg-gray-50 opacity-60">
                                                    <div class="flex items-center justify-between mb-2">
                                                        <div class="flex items-center space-x-3">
                                                            <span
                                                                class="px-2 py-1 text-xs font-bold bg-gray-200 text-gray-500 rounded">
                                                                {{ $item['critere']->numero_critere_evaluation }}
                                                            </span>
                        ...
                        <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                            ...
                            <div class="p-6">
                                <div class="mb-4">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        <input type="number" required name="resultat_evaluation" id="resultatEvaluation"
                                            ...
                                            min="1" step="0.01" max="{{ $resteAEvaluer ?? '' }}"
                                            value="{{ old('resultat_evaluation', 0) }}" required>
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="space-y-2">
                                        <input type="text" name="respo_technique[nom_complet]"
                                            ...
                                            placeholder="Nom complet *" value="{{ old('respo_technique.nom_complet') }}"
                                            required>
                                        ...
                                        <input type="email" name="respo_technique[email]"
                                            ...
                                            placeholder="Email" value="{{ old('respo_technique.email') }}">
                                        <input type="text" name="respo_technique[telephone]"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-400"
                                            placeholder="Téléphone" value="{{ old('respo_technique.telephone') }}">
                                    </div>
                                </div>
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="space-y-2">
                                        <input type="text" name="superviseur[nom_complet]"
                                            ...
                                            placeholder="Nom complet *" value="{{ old('superviseur.nom_complet') }}"
                                            required>
                                        ...
                                        <input type="email" name="superviseur[email]"
                                            ...
                                            placeholder="Email" value="{{ old('superviseur.email') }}">
                                        <input type="text" name="superviseur[telephone]"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-purple-400"
                                            placeholder="Téléphone" value="{{ old('superviseur.telephone') }}">
                                    </div>
                                </div>
                                ...
                                <div class="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div class="space-y-2">
                                        <input type="text" name="evalue_par[nom_complet]"
                                            ...
                                            placeholder="Nom complet *" value="{{ old('evalue_par.nom_complet') }}" required>
                                        ...
                                        <input type="email" name="evalue_par[email]"
                                            ...
                                            placeholder="Email" value="{{ old('evalue_par.email') }}">
                                        <input type="text" name="evalue_par[telephone]"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-green-400"
                                            placeholder="Téléphone" value="{{ old('evalue_par.telephone') }}">
                                    </div>
                                </div>
...
<script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let currentMax = Number("{{ $resteAEvaluer ?? 0 }}");
            let currentNoteRef = Number("{{ $critereSelectionne?->note_reference_critere_evaluation ?? 0 }}");
            // let currentMax = parseFloat(@json($resteAEvaluer)) || 0;
            // let currentNoteRef = parseFloat(@json($critereSelectionne?->note_reference_critere_evaluation)) || 0;
