                ...
                <div class="block lg:hidden divide-y divide-gray-200">
                    @foreach($factures as $facture)
                        ...
                        <div class="p-4 hover:bg-gray-50">
                            ...
                            <div class="flex items-start justify-between mb-3 cursor-pointer" onclick="toggleMobileCard('mobile-{{ $facture->id_facture }}')">
                                ...
                                <div class="flex items-center space-x-2">
                                    ...
                                    <button type="button" class="p-1 text-gray-400" id="mobile-chevron-{{ $facture->id_facture }}">
                            ...
                            <div id="mobile-{{ $facture->id_facture }}" class="hidden mt-3 pt-3 border-t border-gray-200">
                                <div class="grid grid-cols-2 gap-3 mb-3">
                                    <!-- Paiements -->
                                    <div class="bg-emerald-50 rounded-lg p-3">
                                        <div class="flex items-center space-x-2 mb-1">
                                            <i class="fas fa-money-check-alt text-emerald-600 text-sm"></i>
                                            <span class="text-xs text-emerald-700 uppercase">Total de Paiements effectués</span>
                                        </div>
                                    ...
                                    <div class="bg-blue-50 rounded-lg p-3">
                                        ...
                                        @if($lot)
                                            <a href="{{ route('lots.show', $lot->id_lot) }}" class="text-sm font-bold text-blue-800 hover:underline">
                                                {{ $lot->numero }}
                                            </a>
                                        @else
                                    ...
                                    <div class="bg-purple-50 rounded-lg p-3">
                                        ...
                                        @if($prestataire)
                                            <a href="{{ route('prestataires.show', $prestataire->id_prestataire) }}" class="text-sm font-bold text-purple-800 hover:underline truncate block" title="{{ $prestataire->raison_sociale_prestataire }}">
                                    ...
                                    <div class="bg-orange-50 rounded-lg p-3">
                                        ...
                                        @if($attribution)
                                            <a href="{{ route('attributions.show', $attribution->id_attribution) }}" class="text-sm font-bold text-orange-800 hover:underline">
                ...
                <div class="hidden lg:block overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider w-10"></th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">N° Facture</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Proforma</th>
                                ...
                                <th class="px-4 py-3 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Paiement</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($factures as $facture)
                                ...
                                <!-- Ligne principale -->
                                <tr class="hover:bg-gray-50 transition-colors cursor-pointer" onclick="toggleRow('{{ $facture->id_facture }}')">
                                    <td class="px-4 py-4 whitespace-nowrap">
                                        <button type="button" class="p-1 text-gray-400 hover:text-gray-600 transition-colors" id="chevron-{{ $facture->id_facture }}">
                                ...
                                <tr id="details-{{ $facture->id_facture }}" class="hidden bg-gradient-to-r from-gray-50 to-indigo-50">
                                        ...
                                        <div class="py-4 pl-14 pr-4">
                                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                ...
                                                <div class="bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
                                                    <div class="flex items-center space-x-3">
                                                        ...
                                                        <div>
                                                            <p class="text-xs text-gray-500 uppercase tracking-wide">Total de Paiements effectués</p>
                                                            <p class="text-lg font-bold text-gray-900">{{ $nbPaiements }}</p>
                                                        </div>
                                                    </div>
                                                    @if($nbPaiements > 0)
                                                        <a href="{{ route('factures.show', $facture->id_facture) }}#paiements"
                                                           class="mt-2 inline-flex items-center text-xs text-emerald-600 hover:text-emerald-800 transition-colors">
                                                            <i class="fas fa-external-link-alt mr-1"></i> Voir les paiements
                                                        </a>
                                                    @endif
                                                </div>
                                                ...
                                                <div class="bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
                                                    <div class="flex items-center space-x-3">
                                                        ...
                                                        <div class="min-w-0 flex-1">
                                                            <p class="text-xs text-gray-500 uppercase tracking-wide">Lot</p>
                                                            @if($lot)
                                                                <p class="text-sm font-bold text-gray-900 truncate" title="{{ $lot->libelle }}">{{ $lot->numero }}</p>
                                                            @else
                                                ...
                                                <div class="bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
                                                    <div class="flex items-center space-x-3">
                                                        ...
                                                        <div class="min-w-0 flex-1">
                                                            <p class="text-xs text-gray-500 uppercase tracking-wide">Prestataire</p>
                                                            @if($prestataire)
                                                                <p class="text-sm font-bold text-gray-900 truncate" title="{{ $prestataire->raison_sociale_prestataire }}">
                                                    ...
                                                    @if($prestataire)
                                                        <a href="{{ route('prestataires.show', $prestataire->id_prestataire) }}"
                                                           class="mt-2 inline-flex items-center text-xs text-purple-600 hover:text-purple-800 transition-colors">
                                                            <i class="fas fa-external-link-alt mr-1"></i> Voir le prestataire
                                                        </a>
                                                    @endif
                                                </div>
                                                ...
                                                <div class="bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
                                                    <div class="flex items-center space-x-3">
                                                        ...
                                                        <div class="min-w-0 flex-1">
                                                            <p class="text-xs text-gray-500 uppercase tracking-wide">Attribution</p>
                                                            @if($attribution)
                                                                <p class="text-sm font-bold text-gray-900 truncate">{{ $attribution->numero_attribution }}</p>
                                                            @else
                                                    ...
                                                    @if($attribution)
                                                        <a href="{{ route('attributions.show', $attribution->id_attribution) }}"
                                                           class="mt-2 inline-flex items-center text-xs text-orange-600 hover:text-orange-800 transition-colors">
                                                            <i class="fas fa-external-link-alt mr-1"></i> Voir l'attribution
                                                        </a>
                                                    @endif
                                                </div>
                                            </div>
                                            ...
                                            @if($lot && $lot->libelle)
                                                <div class="mt-3 p-3 bg-white rounded-lg border border-gray-200">
                                                    <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">Libellé du lot</p>
