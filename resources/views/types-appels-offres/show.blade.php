        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="flex items-center space-x-2">
                ...
                <button onclick="toggleStatus('{{ $typeAO->id_type_appel_offre }}', {{ $typeAO->actif_type_appel_offre ? 'true' : 'false' }})"
                    ...
                    <span class="text-sm font-medium">{{ $typeAO->actif_type_appel_offre ? 'Désactiver' : 'Activer' }}</span>
                </button>

                <button onclick="confirmDelete('{{ $typeAO->id_type_appel_offre }}', '{{ $typeAO->libelle_type_appel_offre }}', {{ $typeAO->appel_offres_count }})"
                    ...
                    <div class="mt-6">
                        <div class="relative pt-1">
                            ...
                            <div class="overflow-hidden h-3 text-xs flex rounded-full bg-gray-200">
                                <div class="w-full bg-gradient-to-r from-blue-500 via-purple-500 to-green-500 rounded-full"></div>
                            </div>
                            <div class="flex justify-between text-xs text-gray-500 mt-2">
                                <span>{{ number_format($typeAO->valeur_minimuim_type_appel_offre, 0, ',', ' ') }} FCFA</span>
                                <span>{{ number_format($typeAO->valeur_maximuim_type_appel_offre, 0, ',', ' ') }} FCFA</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-lg overflow-hidden">
                <div class="px-6 py-4 bg-gradient-to-r from-purple-50 to-white border-b border-gray-200">
                    <div class="flex items-center justify-between">
                        <h2 class="text-lg font-bold text-gray-800 flex items-center">
                            ...
                            Appels d'offres associés
                            <span class="ml-2 px-2.5 py-1 bg-purple-100 text-purple-800 text-sm font-semibold rounded-full">
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 hover:bg-gray-100 rounded-lg transition-all duration-200">
                            <div class="flex-1">
                                <div class="flex items-center space-x-3">
                                    <span class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-semibold bg-orange-100 text-orange-700">
                                        {{ $ao->numero_appel_offre }}
                                    </span>
                                    <p class="font-medium text-gray-900">{{ $ao->libelle_critere_appel_offre }}</p>
                                </div>
                                <div class="flex items-center space-x-4 mt-2 text-xs text-gray-500">
                                    <span><i class="fas fa-coins mr-1"></i>{{ number_format($ao->montant_global_appel_offre, 0, ',', ' ') }} FCFA</span>
                                    <span><i class="fas fa-calendar mr-1"></i>{{ $ao->created_at->format('d/m/Y') }}</span>
                                </div>
                            </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-transparent rounded-lg border-l-4 border-green-500">
                        <div>
                    ...
                    <!-- Plage utilisée -->
                    <div class="p-4 bg-gradient-to-r from-purple-50 to-transparent rounded-lg border-l-4 border-purple-500">
...
<div id="formModal" class="hidden fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full transform transition-all">
            ...
            <!-- Form -->
            <form id="typeForm" method="POST" action="{{ route('types-appels-offres.update', $typeAO->id_type_appel_offre) }}" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number"
                                   name="valeur_minimuim_type_appel_offre"
                                   id="valeur_min"
                                   required
                                   min="0"
                                   step="0.01"
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="valeur_maximuim_type_appel_offre"
                                   id="valeur_max"
                                   required
                                   min="0"
                                   step="0.01"
                    ...
                    <div>
                        ...
                        <textarea name="description_critere_type_appel_offre"
                                  id="description"
                                  rows="4"
        ...
        if (data.success) {
            ...
            if (data.errors) {
                Object.keys(data.errors).forEach(key => {
                    const errorDiv = document.getElementById(`error_${key.replace('_type_appel_offre', '').replace('_critere', '')}`);
                    if (errorDiv) {
